...
if (count($errors) > 0) {
  ...
  try {
    ...
    $statement = $pdo->prepare("UPDATE emp SET fname = ?, lname = ?, gender = ?, email = ?, address = ?, salary = ?, password = ?, image = ? WHERE id = $id ");     

    $statement->execute([$fname, $lname, $gender, $email, $address, $salary, $password, $imageBaseName]);
