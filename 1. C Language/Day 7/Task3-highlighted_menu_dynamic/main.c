...
{
    do {
        printf("Please enter a valid no of employees to store: ");
        scanf("%d", &size);
    } while (size <= 0);

    idArray = (int*) malloc(size * sizeof(int));
    indexArray = (int*) malloc(size * sizeof(int));
    emps = (struct Emp *) malloc(size * sizeof(struct Emp));
